...
- ~~重构支付链路为事件驱动架构~~ — 已否决：改动范围过大，短期 ROI 不足`,
      chapters: [
        { id: 'ch6_1', timestamp: '14:00', title: '会议开场与故障概述', summary: '主持人介绍会议目标：复盘 5月13日支付系统故障，明确根因与改进措施。参会人员：SRE、后端负责人、支付负责人、安全工程师。' },
        { id: 'ch6_2', timestamp: '14:05', title: '故障时间线回顾', summary: 'SRE 团队还原故障全过程：14:22 告警触发 → 14:25 开始降级 → 14:38 定位根因（Redis 连接池耗尽）→ 14:52 恢复 → 15:10 全量恢复，总持续约 50 分钟。' },
        { id: 'ch6_3', timestamp: '14:15', title: '影响范围评估', summary: '受影响订单 8.7 万笔，涉及金额约 1200 万元，用户投诉 2300+ 条，客服工单 1500+。支付成功率从 99.9% 降至 67.3%。' },
        { id: 'ch6_4', timestamp: '14:25', title: '根因分析', summary: 'DBA 定位核心根因：Redis 连接池 max_connections=100，在高峰时段（14:00-14:30）并发请求达 500+，连接池耗尽后所有支付请求阻塞超时。' },
        { id: 'ch6_5', timestamp: '14:40', title: '修复方案讨论', summary: '方案 A：调整 Redis 连接池至 500 + 增加水位监控（SRE 提出，1 周上线）；方案 B：支付链路增加熔断降级（后端负责人提出，2 周上线）；方案 C：重构支付链路为事件驱动架构（架构师提出，2-3 个月，被否决）。' },
        { id: 'ch6_6', timestamp: '15:00', title: '决策与排期', summary: '最终采纳方案 A + B 组合。方案 A 本周五前上线，方案 B 下周三前完成设计。方案 C 因改动范围过大留待 Q3 评估。' },
      ],
      decisions: [
    ...
    todos: [
      { id: 't6_1', content: 'Redis 连接池参数调优上线', assignee: '后端负责人', dueDate: '2026-05-17', status: 'completed', contextSnippet: 'Redis 连接池 max_connections=100，高峰时段并发请求达 500+，连接池耗尽导致支付超时' },
      { id: 't6_2', content: '新增连接池水位监控告警（阈值 80%）', assignee: 'SRE', dueDate: '2026-05-21', status: 'in_progress', contextSnippet: '故障期间无连接池水位预警，需建立监控机制提前发现问题' },
      { id: 't6_3', content: '完成全链路压测验证', assignee: 'SRE', dueDate: '2026-05-21', status: 'pending', contextSnippet: '连接池调整后需在灰度环境完成压测，验证 500 连接数是否满足峰值需求' },
      { id: 't6_4', content: '支付链路熔断降级方案设计', assignee: '后端负责人', dueDate: '2026-05-21', status: 'pending', contextSnippet: '支付链路需增加熔断降级机制，防止类似级联故障再次发生' },
      { id: 't6_5', content: '完善故障应急 SOP 文档', assignee: 'SRE', dueDate: '2026-05-28', status: 'pending', contextSnippet: '本次故障响应耗时 30 分钟定位根因，需优化 SOP 缩短响应时间' },
      { id: 't6_6', content: '用户补偿方案评估', assignee: '支付负责人', dueDate: '2026-05-21', status: 'pending', contextSnippet: '受影响 8.7 万笔订单约 1200 万元，需评估用户补偿策略' },
    ],
    preTodos: [],
